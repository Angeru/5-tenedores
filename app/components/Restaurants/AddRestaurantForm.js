...
export default function AddRestaurantForm(props){
    ...
    const AddRestaurant = ()=>{
        if(!restaurantName || !restaurantAddress || !restaurantDescription){
            ...
            toastRef.current.show("Tiene que subir al menos una foto")
        ...
        await Promise.all(
            map(imageSelected, async image=>{
                ...
                await ref.put(blob).then(async(result) =>{
                    await firebase.storage().ref(`restaurants/${result.metadata.name}`).getDownloadURL()
                    .then(photoUrl => {
...
function UploadImage(props){
    ...
    if(resultPermissions =="denied"){
        ...
        if(result.cancelled){
            toastRef.current.show("No has seleccionado niguna imagen",2000)
        }else{
